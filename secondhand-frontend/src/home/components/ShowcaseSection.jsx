...
const ShowcaseSection = () => {
  ...
  return (
    <section ref={sectionRef} className="bg-white py-12 border-b border-gray-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-2">Featured Listings</h2>
        ...
        {!loadShowcases ? (
          ...
          <div className={`grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            {showcases.slice(0, 6).map((showcase) => (
              <Link
                key={showcase.id}
                to={`${ROUTES.LISTINGS}/${showcase.listingId}`}
                className="group bg-white border border-gray-200 rounded-md shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden"
              >
                <div className="aspect-square bg-gray-100 relative overflow-hidden">
                  ...
                  <div className="absolute top-2 right-2 bg-gray-900 text-white px-1.5 py-0.5 rounded text-[10px] font-semibold">
                    Featured
                  </div>
                </div>
