...
const HeroListingCard = ({ listing, style, className = "" }) => {
  ...
  return (
    <div
      onClick={handleClick}
      style={style}
      className={`w-80 bg-white border border-slate-900/5 rounded-2xl p-4 shadow-[0_4px_24px_-4px_rgba(15,23,42,0.06),0_1px_3px_-1px_rgba(15,23,42,0.02)] cursor-pointer transition-all duration-300 hover:border-slate-200 hover:-translate-y-1 hover:shadow-[0_20px_35px_-10px_rgba(15,23,42,0.08)] select-none ${className}`}
    >
      <div className="relative aspect-video rounded-xl bg-slate-50 overflow-hidden border border-slate-100 flex items-center justify-center">
        {listing.imageUrl ? (
          ...
          <div className="text-slate-300 text-xs font-semibold">No Image</div>
        )}
        <div className="absolute top-2.5 left-2.5 flex flex-col gap-1 z-10">
          ...
          {isAuraApproved && (
            <span className="px-2 py-0.5 rounded bg-indigo-600 text-[9px] font-bold text-white uppercase tracking-wider shadow-sm flex items-center gap-0.5">
              <Sparkles className="w-2.5 h-2.5" /> Aura Pick
            </span>
          )}
        </div>
      </div>
