...
export const CommentItem = ({
  ...
  return <div style={{
    marginLeft: indent > 0 ? `${indent * 20}px` : undefined
      ...
      <div className={`rounded-xl transition-colors duration-200 ${isNested ? 'border-l-2 border-gray-200 pl-4' : ''}`}>
        <div className="py-3">
          {/* Author row */}
          <div className="flex items-start gap-3">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 text-xs font-bold ${isNested ? 'bg-gray-100 text-gray-500' : 'bg-gradient-to-br from-violet-100 to-indigo-100 text-violet-700'}`}>
              {authorInitial}
            </div>
            <div className="flex-1 min-w-0">
              ...
              <div className="mt-2.5 flex items-center gap-1">
                {/* Compact vote */}
                <button type="button" className={`inline-flex items-center justify-center w-7 h-7 rounded-md transition-all duration-200 ${voteUpActive ? 'text-violet-700 bg-violet-50 ring-1 ring-violet-200/60' : 'text-gray-400 hover:text-violet-600 hover:bg-violet-50'}`} onClick={() => onReact?.(threadId, comment?.id, FORUM_REACTIONS.LIKE)}>
                  <ChevronUp className="w-4 h-4" />
                </button>
                <span className={`text-xs font-bold tabular-nums min-w-[16px] text-center ${netVotes > 0 ? 'text-violet-600' : netVotes < 0 ? 'text-red-500' : 'text-gray-400'}`}>
                  {netVotes}
                </span>
                <button type="button" className={`inline-flex items-center justify-center w-7 h-7 rounded-md transition-all duration-200 ${voteDownActive ? 'text-red-600 bg-red-50 ring-1 ring-red-200/60' : 'text-gray-400 hover:text-red-500 hover:bg-red-50'}`} onClick={() => onReact?.(threadId, comment?.id, FORUM_REACTIONS.DISLIKE)}>
                  <ChevronDown className="w-4 h-4" />
                </button>

                <div className="w-px h-4 bg-gray-200 mx-1.5" />

                <button type="button" className="inline-flex items-center gap-1 text-xs text-gray-500 hover:text-violet-600 font-medium px-2 py-1.5 rounded-md hover:bg-violet-50 transition-all duration-200" onClick={() => onReply?.(comment?.id)}>
                  <CornerDownRight className="w-3 h-3" />{t("reply")}</button>
              </div>

              {/* Reply composer (inline) */}
              {isReplyTarget && <div className="mt-3 rounded-xl border border-gray-200 bg-gray-50/60 overflow-hidden focus-within:border-violet-300 focus-within:ring-2 focus-within:ring-violet-500/10 transition-all duration-200">
                  ...
                  <div className="px-3 py-2.5 border-t border-gray-200/60 bg-white flex items-center justify-end gap-2">
                    <button type="button" onClick={onCancelReply} className="px-3 py-1.5 rounded-lg text-xs font-semibold text-gray-600 hover:bg-gray-100 transition-colors duration-200">{t("cancel")}</button>
