          ...
          <div className="py-6 sm:py-8">
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              {isAuthenticated && <div className="flex items-center gap-2">
                  <button type="button" onClick={() => setSettingsOpen(true)} className="h-10 w-10 rounded-xl border border-gray-200 bg-white text-gray-500 hover:text-gray-700 hover:bg-gray-50 hover:border-gray-300 inline-flex items-center justify-center transition-all duration-200" title={t("forum_settings")}>
            ...
            <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className={`relative w-full sm:max-w-sm transition-all duration-300 ${searchFocused ? 'sm:max-w-md' : ''}`}>
                ...
                <input value={forum.search} onChange={e => forum.setSearch(e.target.value)} onFocus={() => setSearchFocused(true)} onBlur={() => setSearchFocused(false)} placeholder={t("search_threads")} className="w-full h-10 pl-10 pr-4 rounded-xl border border-gray-200 bg-gray-50/80 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-400 focus:bg-white transition-all duration-200" />
              </div>

              {/* Sort */}
              <div className="flex items-center gap-2">
                <ArrowUpDown className="w-3.5 h-3.5 text-gray-400" />
                <select value={forum.sort} onChange={e => forum.setSort(e.target.value)} className="rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700 font-medium focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-400 cursor-pointer transition-all duration-200">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 xl:col-span-4">
            <div className="rounded-2xl border border-gray-200/70 bg-white/70 backdrop-blur-sm p-2 shadow-sm lg:sticky lg:top-6 space-y-2 lg:max-h-[calc(100vh-8rem)] lg:overflow-y-auto lg:pr-1 [scrollbar-width:thin]">
              {forum.threadsError && <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 font-medium">
                  {forum.threadsError}
                </div>}
              ...
              {showListSkeleton ? <>
                  ...
                  <ThreadCardSkeleton />
                  <ThreadCardSkeleton />
                </> : visibleThreads.length === 0 ? <div className="rounded-2xl border border-gray-200/90 bg-white/90 shadow-sm ring-1 ring-gray-100 p-12 text-center mx-0.5">
                  ...
                  {forum.threadsHasMore && listTab !== FORUM_LIST_TABS.LIKED && <div className="flex justify-center pt-3 pb-1">
                      <button type="button" onClick={forum.loadMoreThreads} disabled={forum.threadsLoading} className="px-5 py-2.5 rounded-xl border border-gray-200 bg-white text-sm font-semibold text-gray-700 hover:bg-gray-50 hover:border-gray-300 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
