...
const Panel = ({ children, className = '' }) => (
  <div
    className={`rounded-3xl border border-slate-200/80 bg-white/80 backdrop-blur-md shadow-[0_8px_30px_rgb(0,0,0,0.06)] ${className}`}
  >
    {children}
  </div>
);
...
const OrderShipmentPage = () => {
  ...
  const { data: order, isLoading, error, refetch } = useQuery({
    ...
    queryFn: () =>
      isSellerView
        ? orderService.getSellerOrderById(orderId)
  ...
  return (
    <div className="min-h-[calc(100vh-8rem)] bg-gradient-to-b from-slate-50 via-white to-indigo-50/30 pb-16">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 pt-8 sm:pt-10 space-y-6">
        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={() => navigate(backPath)}
            className="inline-flex items-center gap-2 text-sm font-semibold text-slate-600 hover:text-slate-900 py-2"
          >
        ...
        <div className="space-y-1">
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-slate-900">Shipment tracking</h1>
        ...
        {isLoading ? (
          <Panel className="p-12 flex flex-col items-center justify-center animate-pulse">
            <Package className="w-10 h-10 text-slate-300 mb-3" />
            <span className="text-sm font-medium text-slate-400">Loading shipment…</span>
          </Panel>
        ) : null}

        {errorMessage && !isLoading ? (
          <Panel className="p-6 border-rose-200 bg-rose-50/90">
            <p className="text-sm font-semibold text-rose-800">{errorMessage}</p>
            <button
              type="button"
              className="mt-3 text-sm font-semibold text-indigo-600 hover:text-indigo-800 underline"
              onClick={() => refetch()}
            >
              Try again
            </button>
        ...
        {order && !isLoading ? (
          <>
            {order.orderItems?.length ? (
              <Panel className="p-5">
                <h2 className="text-[11px] font-semibold text-slate-500 uppercase tracking-widest mb-3">Items</h2>
                <ul className="space-y-2">
                  {order.orderItems.map((item) => (
                    <li key={item.id ?? `${item.listing?.id}-${item.quantity}`} className="flex justify-between gap-3 text-sm">
            ...
            {order.shipping ? (
              ...
              <Panel className="p-10 text-center">
                ...
                <p className="text-sm font-semibold text-slate-900">No shipment yet</p>
                <p className="text-sm text-slate-500 mt-1">Shipping details appear after the seller dispatches.</p>
              </Panel>
            )}
            ...
            {order.shipping?.trackingNumber ? (
              <div className="flex flex-wrap items-center gap-3">
                <button
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-slate-200 bg-white text-xs font-semibold text-slate-700 hover:bg-slate-50"
                >
