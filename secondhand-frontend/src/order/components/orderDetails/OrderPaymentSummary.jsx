...
export const OrderPaymentSummary = React.memo(({
  ...
  return (
    <CardComponent critical className={`${isSellerView ? 'p-6' : 'p-5'} border border-white/5`}>
        ...
        <div className="mb-5 pb-5 border-b border-white/10">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 px-2.5 py-1 rounded-full bg-black/20 border border-white/5">
              ...
              <span className={`text-[11px] uppercase tracking-wider font-bold ${getPaymentStatusTextClass(selectedOrder.paymentStatus)}`}>
      ...
      {selectedOrder.paymentReference && onOpenReceipt ? (
        <button
          ...
          className="w-full mt-5 py-3 bg-white/5 hover:bg-white/10 rounded-xl text-xs font-bold tracking-wide transition-all flex items-center justify-center gap-2 border border-white/10 hover:border-white/25 text-slate-300 hover:text-white group"
        >
