...
const BREAKDOWN = [
  {key: 'fiveStarReviews', label: '5★', bar: 'bg-amber-400'},
  {key: 'fourStarReviews', label: '4★', bar: 'bg-amber-300'},
  {key: 'threeStarReviews', label: '3★', bar: 'bg-slate-300'},
  {key: 'twoStarReviews', label: '2★', bar: 'bg-slate-400'},
  {key: 'oneStarReviews', label: '1★', bar: 'bg-slate-500'},
];
...
const StarBreakdown = ({stats}) => {
  ...
  return (
    <div className="space-y-2.5 mt-6">
      <p className="text-[11px] font-semibold uppercase tracking-wider text-gray-400">Rating mix</p>
...
const UserReviews = ({
  ...
  const filterTotalPages =
    filteredAll.length > 0 ? Math.max(1, Math.ceil(filteredAll.length / clientPerPage)) : 1;
  ...
  const cycleFilterNext = useCallback(
    () => setFilterPage((fp) => Math.min(filterTotalPages - 1, fp + 1)),
    [filterTotalPages]
  );
    ...
    <div className="space-y-6 min-w-0 max-w-full">
      <section className="flex min-w-0 max-w-full flex-col overflow-visible rounded-2xl border border-gray-200/90 bg-white shadow-sm">
        <div className="shrink-0 min-w-0 p-5 sm:p-7 bg-gradient-to-br from-gray-50 via-white to-amber-50/30 border-b border-gray-100">
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6 lg:gap-10 min-w-0 w-full">
            <div className="min-w-0">
              ...
              <h2 className="text-xl sm:text-2xl font-bold text-gray-900 tracking-tight">Reputation</h2>
              <p className="text-sm text-gray-500 mt-2 max-w-md leading-relaxed">
                {isOwnProfile
                  ? 'Ratings left by buyers after completed orders. Build trust with clear communication and accurate listings.'
                  : 'Aggregated from reviews this member received as a seller.'}
              </p>
            </div>
              ...
              {totalReviewsStat > 0 ? (
                <>
                  ...
                  <p className="text-sm text-gray-500 mt-1 tabular-nums">
                    Based on <strong className="text-gray-800 font-semibold">{totalReviewsStat}</strong>{' '}
                    {totalReviewsStat === 1 ? 'review' : 'reviews'}
                  </p>
                </>
              ) : (
                <p className="text-sm text-gray-500 py-2">No reviews yet — average will show after the first rating.</p>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 min-w-0">
            <div className="min-w-0 flex-1">
              <h3 className="text-base font-bold text-gray-900 tracking-tight">
                Reviews received
                {!filterActive && pagination?.totalElements > 0 ? (
                  <span className="font-normal text-gray-400 ml-2 tabular-nums">({pagination.totalElements})</span>
                ) : null}
                {filterActive ? (
                  <span className="font-normal text-gray-400 ml-2 tabular-nums">
                    ({filteredAll.length} for selected listing)
                  </span>
                ) : null}
              </h3>
              <p className="text-xs text-gray-500 mt-1">
                {filterActive
                  ? 'Filtered on this browser · Scroll down for reviews'
                  : 'Newest first • Public to profile visitors'}
              </p>
            </div>
            {!initialListLoading && !filterActive && pagination?.totalElements > 0 ? (
              <label className="inline-flex items-center gap-2 text-sm text-gray-600 shrink-0">
                <span className="hidden sm:inline">Per page</span>
                <select
                  value={pagination.size}
                  onChange={(e) => handlePageSizeChange(Number(e.target.value))}
                  className="h-10 min-w-[88px] rounded-xl border border-gray-200 bg-white px-3 text-sm font-medium text-gray-800 shadow-sm focus:outline-none focus:ring-2 focus:ring-gray-900/15 focus:border-gray-400 cursor-pointer"
                >
          ...
          {totalReviewsStat > 0 ? (
            <div className="min-w-0">
              <p className="text-[11px] font-semibold uppercase tracking-wide text-gray-400 mb-2 flex items-center gap-1.5">
                ...
                Filter by listing
                {indexFetching ? (
                  <Loader2 className="w-3.5 h-3.5 animate-spin text-amber-700" aria-hidden />
                ) : null}
              </p>
              <div
                className="min-w-0 w-full overflow-x-auto overflow-y-hidden overscroll-x-contain [-webkit-overflow-scrolling:touch] touch-pan-x pb-2 -mx-1 px-1 [scrollbar-width:thin]"
                role="group"
                aria-label="Listing filter chips"
              >
                <div className="flex flex-row flex-nowrap gap-2 w-max max-w-none pr-1">
                  <button
                    type="button"
                    onClick={() => setListingFilter(ALL_LISTINGS)}
                    className={`flex-none inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-semibold transition-colors max-w-[min(240px,calc(100vw-120px))] ${
                      !filterActive
                  ...
                  >
                    ...
                    {totalReviewsStat > 0 ? (
                      <span
                        className={`tabular-nums px-2 py-0.5 rounded-md text-[10px] ${!filterActive ? 'bg-white/15' : 'bg-gray-100'}`}
                      >
                        {totalReviewsStat}
                      </span>
                    ) : null}
                  ...
                  {listingOptions.map((opt) => {
                    ...
                    return (
                      <button
                        key={opt.key}
                        type="button"
                        onClick={() => setListingFilter(opt.key)}
                        title={opt.title}
                        className={`flex-none max-w-none text-left inline-flex flex-col rounded-xl border px-3 py-2 text-[11px] font-semibold transition-colors min-w-[10.5rem] max-w-[min(288px,calc(100vw-6rem))] ${
                          active
                            ? 'border-amber-500 bg-amber-50 text-amber-950 ring-2 ring-amber-200'
                            : 'border-gray-200 bg-white text-gray-800 hover:border-amber-200'
                        }`}
                      >
                        ...
                        <span className="mt-1 flex flex-wrap items-center gap-1 text-[10px] font-semibold uppercase tracking-wide text-gray-400">
                          ...
                          <span
                            className={`tabular-nums px-2 py-0.5 rounded-md ${
                              active ? 'bg-white/70 text-amber-900' : 'bg-gray-100 text-gray-500'
                            }`}
                          >
                            {opt.count} reviews
                          </span>
              ...
              {indexLoadError ? (
                <p className="mt-2 text-[11px] text-amber-800">Listing filter incomplete: {indexLoadError}</p>
              ) : null}
            </div>
          ) : null}
        </div>
        ...
        <div className="relative bg-gray-50/40">
          {pagingLoading && !filterActive ? (
            <div
              className="pointer-events-none absolute inset-x-0 top-0 z-[1] h-0.5 bg-gradient-to-r from-transparent via-amber-400 to-transparent opacity-80 animate-pulse"
              aria-hidden
            />
          ) : null}

          {initialListLoading ? (
            <div className="p-5 sm:p-7" aria-busy="true">
              <ul className="space-y-4">
                {[...Array(4)].map((_, i) => (
                  <li key={i} className="rounded-2xl border border-gray-200/80 bg-white p-5 shadow-sm animate-pulse">
            ...
            <div className="p-5 sm:p-7">
              <div className="rounded-2xl border border-red-100 bg-red-50/50 px-6 py-14 text-center">
                ...
                <p className="mb-1 text-base font-semibold text-gray-900">Couldn&apos;t load reviews</p>
                <p className="text-sm text-gray-600">{receivedReviewsError}</p>
              </div>
            </div>
          ) : receivedReviews.length === 0 && !filterActive ? (
            <div className="p-5 sm:p-7">
              <div className="rounded-2xl border border-dashed border-gray-200 bg-white px-6 py-16 text-center">
                <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-gray-100">
                  <Star className="h-8 w-8 text-gray-300" strokeWidth={1.25} />
                </div>
                <h4 className="mb-2 text-lg font-bold text-gray-900">No reviews received yet</h4>
                <p className="mx-auto max-w-sm text-sm leading-relaxed text-gray-500">
                  {isOwnProfile
                    ? 'When buyers complete orders and leave feedback, ratings will appear here.'
                    : 'This member has not received any public reviews.'}
                </p>
                {isOwnProfile ? (
                  <Link
                    to={ROUTES.MY_LISTINGS}
                    className="mt-6 inline-flex text-sm font-semibold text-gray-900 underline underline-offset-4 hover:text-amber-800"
                  >
                    Go to my listings
                  </Link>
                ) : null}
            ...
            <div className="p-5 sm:p-7">
              <ul
                className={`space-y-4 pb-1 ${
                  filterActive ? '' : pagingLoading ? 'opacity-70 transition-opacity duration-150' : ''
                }`}
              >
                {displayReviews.length === 0 && filterActive ? (
                  <li className="rounded-2xl border border-dashed border-gray-200 bg-white px-6 py-12 text-center text-sm text-gray-600">
                    No reviews matched this listing — try reloading the profile tab or pick another listing chip.
                  </li>
                ) : (
              ...
              {!filterActive && pagination && pagination.totalPages > 1 ? (
                <nav
                  className="mt-6 flex shrink-0 flex-col gap-4 border-t border-gray-200/90 bg-gray-50/40 pb-6 pt-4 sm:flex-row sm:items-center sm:justify-between"
                  aria-label="Reviews pagination"
                >
                  <p className="order-2 text-xs tabular-nums text-gray-500 sm:order-1">
                    Page {page + 1} of {pagination.totalPages}
                    {pagination.totalElements ? (
                      <span className="text-gray-400"> · {pagination.totalElements} total</span>
                    ) : null}
                  </p>
                  <div className="order-1 flex items-center justify-center gap-2 sm:order-2">
                    <button
                      ...
                      disabled={page <= 0 || pagingLoading}
                      className="inline-flex h-11 items-center gap-1.5 rounded-xl border border-gray-200 bg-white px-4 text-sm font-semibold text-gray-800 shadow-sm transition-colors hover:bg-gray-50 disabled:pointer-events-none disabled:opacity-40"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => loadPage(page + 1)}
                      disabled={page >= totalPages - 1 || pagingLoading}
                      className="inline-flex h-11 items-center gap-1.5 rounded-xl border border-gray-200 bg-white px-4 text-sm font-semibold text-gray-800 shadow-sm transition-colors hover:bg-gray-50 disabled:pointer-events-none disabled:opacity-40"
                    >
                      Next
                    ...
                    {pagingLoading ? (
                      <Loader2 className="ml-2 h-5 w-5 animate-spin text-amber-600" aria-label="Loading" />
                    ) : null}
                  </div>
                </nav>
              ) : null}

              {filterActive && filteredAll.length > 0 && filterTotalPages > 1 ? (
                <nav
                  className="mt-6 flex shrink-0 flex-col gap-4 border-t border-gray-200/90 bg-amber-50/30 px-0 pb-6 pt-4 sm:flex-row sm:items-center sm:justify-between"
                  aria-label="Filtered reviews pagination"
                >
                  <p className="text-xs tabular-nums text-gray-500">
                    Listing filter · Page {filterPage + 1} of {filterTotalPages} · {filteredAll.length} reviews
                  </p>
                  <div className="flex items-center justify-center gap-2">
                    <button
                      type="button"
                      onClick={cycleFilterPrev}
                      disabled={filterPage <= 0}
                      className="inline-flex h-11 items-center gap-1.5 rounded-xl border border-gray-200 bg-white px-4 text-sm font-semibold text-gray-800 shadow-sm hover:bg-gray-50 disabled:opacity-40"
                    >
                      <ChevronLeft className="h-4 w-4" />
                      Previous
                    </button>
