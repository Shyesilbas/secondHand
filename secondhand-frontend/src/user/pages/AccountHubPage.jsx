    ...
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center">
        <div className="relative flex items-center justify-center">
          <div className="w-12 h-12 rounded-full border-2 border-emerald-600/20 border-t-emerald-600 animate-spin" />
          <Sparkles className="w-5 h-5 text-emerald-600 absolute" />
        </div>
      </div>
    );
  }
        ...
        <div className="bg-white rounded-3xl p-5 border border-slate-200/80 shadow-xs mb-6 transition-all hover:border-slate-300">
          <div className="flex items-center gap-3.5">
            <div className="relative">
              <div className="h-12 w-12 rounded-2xl bg-emerald-600 text-white flex items-center justify-center font-extrabold text-sm shadow-xs overflow-hidden">
                {user?.profilePicture ? (
            ...
            <div className="overflow-hidden min-w-0 flex-1">
              ...
              <div className="mt-1.5 flex items-center gap-1.5">
                <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-extrabold uppercase tracking-wider ${
                  ...
                  <span className={`w-1.5 h-1.5 rounded-full ${isPremium ? 'bg-amber-500' : 'bg-emerald-600'}`} />
                  {isPremium ? (plan || 'Premium') : t('standard_plan', 'Standart')}
                </span>
                <Link 
                  to={ROUTES.PROFILE} 
                  className="text-[10px] font-bold text-slate-400 hover:text-emerald-700 underline underline-offset-2 transition-colors ml-auto"
                >
                  {t('edit', 'Düzenle')}
                </Link>
              </div>
              ...
              <div key={group.id} className="mb-0.5">
                ...
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    <GroupIcon className={`h-4 w-4 shrink-0 ${hasActiveChild ? 'text-emerald-600' : 'text-slate-400'}`} strokeWidth={2} />
      ...
      <main className="flex-1 p-4 sm:p-6 lg:p-8 lg:pl-2 overflow-y-auto">
        <PageContainer className="max-w-5xl">
          ...
          <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <Link
                ...
                className="inline-flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold uppercase tracking-wider shadow-xs hover:shadow-sm transition-all active:scale-[0.98]"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <div className="w-10 h-10 rounded-2xl bg-emerald-50 border border-emerald-100 flex items-center justify-center text-emerald-700 group-hover:scale-105 transition-transform">
                  <Package className="w-5 h-5" />
                </div>
                <span className="text-[10px] font-extrabold text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-full uppercase tracking-wider border border-emerald-200/50">
                  {t('active', 'Aktif')}
                </span>
              </div>
              <div>
            ...
            >
              ...
              <div>
                <div className="flex items-baseline gap-1 text-2xl font-extrabold text-slate-900 font-mono tracking-tight">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80 flex items-center justify-between">
                      <div>
                        ...
                        <span className="text-xs font-extrabold text-emerald-700 mt-0.5 block">{estimatedShippingDays} {t('days_priority', 'Gün (Öncelikli)')}</span>
                      </div>
                    ...
                    <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80 flex items-center justify-between">
                      <div>
                        ...
                        <span className="text-xs font-extrabold text-emerald-700 mt-0.5 block">{orderProcessingSpeed || 'Aynı Gün'}</span>
                      </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {purchaseDate && (
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <span className="font-bold text-slate-900 font-mono">{formatDate(purchaseDate)}</span>
                      </div>
                    )}
                    ...
                    {expirationDate && (
                      <div className="flex items-center justify-between text-xs pt-2 border-t border-slate-200/60">
                        ...
                        <span className="font-bold text-slate-900 font-mono">{formatDate(expirationDate)}</span>
                      </div>
                    )}

                    <div className="flex items-center justify-between text-xs pt-2 border-t border-slate-200/60">
                      <span className="text-slate-500 font-medium">{t("auto_renew", "Otomatik Yenileme")}</span>
                      <span className={`font-bold ${autoRenew ? 'text-emerald-700' : 'text-slate-500'}`}>
                  ...
                  <div className="mt-5 pt-3 border-t border-slate-200/80">
                    {autoRenew ? (
                      ...
                      <button 
                        ...
                        className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold uppercase tracking-wider transition-all disabled:opacity-50 cursor-pointer shadow-xs active:scale-[0.98]"
                      >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-4">
                  ...
                  <div className="bg-slate-50 border border-slate-200/80 p-4 rounded-2xl space-y-2">
                    <div className="flex justify-between text-xs font-bold text-slate-700">
                      ...
                      <span className="font-mono text-slate-900 font-extrabold">{dailyAuraUsage} / {dailyAuraLimit || 2}</span>
                    </div>
                    <div className="h-2 w-full bg-slate-200 rounded-full overflow-hidden">
                      <div 
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80">
                      ...
                      <div className="text-xs font-bold text-slate-700 mt-0.5">
                        {estimatedShippingDays} {t('days', 'Gün')} <span className="text-emerald-700 font-extrabold">({t('with_premium', 'Premium ile')} {premiumEstimatedShippingDays} {t('days', 'Gün')})</span>
                      </div>
                    </div>
                    ...
                    <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80">
                      ...
                      <div className="text-xs font-bold text-slate-700 mt-0.5">
                        {maxShowcaseSlots} {t('slot', 'Slot')} <span className="text-emerald-700 font-extrabold">({t('with_premium', 'Premium ile')} {premiumMaxShowcaseSlots} {t('slot', 'Slot')})</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-b from-slate-900 to-slate-950 text-white p-6 rounded-3xl flex flex-col justify-between shadow-sm relative overflow-hidden">
                  ...
                  <div className="space-y-3 relative z-10">
                    <div className="inline-flex p-2.5 bg-emerald-500/20 text-emerald-400 rounded-2xl border border-emerald-500/30">
                      <Crown className="w-5 h-5 fill-current" />
                    </div>
                    <div>
                      <h4 className="text-xs font-extrabold text-emerald-400 uppercase tracking-wider">{t('why_upgrade_premium', 'Neden Premium?')}</h4>
                  ...
                  <button
                    onClick={() => setShowUpgrade(true)}
                    className="w-full py-3 rounded-2xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-extrabold uppercase tracking-wider text-xs shadow-sm transition-all cursor-pointer mt-5 active:scale-[0.98]"
                  >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
            ...
            >
              <div className="h-11 w-11 shrink-0 rounded-2xl bg-emerald-50 border border-emerald-100 flex items-center justify-center text-emerald-700 group-hover:scale-105 transition-transform">
            ...
            <Link 
              ...
              className="bg-white rounded-3xl p-4 sm:p-5 border border-slate-200/80 shadow-xs flex items-center gap-4 transition-all hover:border-slate-300 hover:shadow-xs group"
            >
              <div className="h-11 w-11 shrink-0 rounded-2xl bg-indigo-50 border border-indigo-100 flex items-center justify-center text-indigo-700 group-hover:scale-105 transition-transform">
          ...
          <div className="bg-white rounded-3xl border border-slate-200/80 shadow-xs p-6 lg:p-7 mb-8">
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-100">
              ...
              <Link 
                to={ROUTES.MY_ORDERS} 
                className="text-xs font-extrabold uppercase tracking-wider text-emerald-700 flex items-center gap-1 hover:text-emerald-800 transition-colors"
              >
            ...
            {ordersLoading ? (
              ...
              <div className="text-center py-12 select-none">
                ...
                <Link 
                  to={ROUTES.LISTINGS} 
                  className="mt-5 inline-flex rounded-xl bg-emerald-600 px-5 py-2.5 text-xs font-extrabold uppercase tracking-wider text-white hover:bg-emerald-700 transition-all shadow-xs"
                >
              ...
              <div className="space-y-3">
                {recentOrders.map(order => {
                  ...
                  const statusConfig = {
                    COMPLETED: { label: t('status_completed', 'Teslim Edildi'), style: 'bg-emerald-50 text-emerald-800 border-emerald-200/60' },
                    DELIVERED: { label: t('status_delivered', 'Teslim Edildi'), style: 'bg-emerald-50 text-emerald-800 border-emerald-200/60' },
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3.5 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <span className="text-[11px] text-slate-400 font-mono font-bold">#{order.orderNumber}</span>
                      ...
                      <div className="text-right shrink-0 ml-3">
                        <p className="text-sm font-extrabold font-mono text-slate-900 tracking-tight">
