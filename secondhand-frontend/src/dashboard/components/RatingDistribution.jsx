...
const RatingDistribution = ({ ratingDistribution = {}, averageRating = 0, totalReviews = 0 }) => {
        ...
        <div className="text-center">
          ...
          <div className="flex items-center gap-0.5 justify-center mt-1">
            {[1, 2, 3, 4, 5].map(star => (
              <Star
                key={star}
                className={`w-3 h-3 ${star <= Math.round(averageRating) ? 'text-amber-400 fill-amber-400' : 'text-slate-200'}`}
              />
            ))}
          </div>
          <p className="text-[10px] text-slate-400 font-semibold mt-1">{totalReviews} reviews</p>
        </div>
