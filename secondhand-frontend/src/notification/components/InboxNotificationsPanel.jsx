...
const InboxNotificationsPanel = () => {
  ...
  if (isLoading) {
    return <div className="flex h-[clamp(540px,min(88vh,920px))] flex-col overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-lg shadow-slate-100/50">
        ...
        <div className="flex flex-1 items-center justify-center px-6 text-sm text-slate-400 font-medium">
          <Loader2 className="w-5 h-5 animate-spin text-indigo-500 mr-2" />{t("bildirimler_y_kleniyor")}</div>
      </div>;
  }
  return <div className="flex h-[clamp(540px,min(88vh,920px))] flex-col overflow-hidden rounded-3xl border border-slate-200/80 bg-white/95 backdrop-blur-xl shadow-lg shadow-slate-100/40 relative">
      {/* Premium Header Bar */}
      <header className="flex h-16 shrink-0 items-center justify-between gap-4 border-b border-slate-100/90 bg-slate-50/70 px-5 sm:px-6 relative z-10">
        <div className="flex min-w-0 items-center gap-3">
          ...
          {unreadCount > 0 && <span className="hidden sm:inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-indigo-600 text-white shadow-sm shadow-indigo-900/10">
              {unreadCount > 99 ? '99+' : unreadCount}{t("yeni")}</span>}
        </div>

        <div className="flex shrink-0 items-center gap-2">
          <button type="button" onClick={() => refetch()} className="p-2 text-slate-400 hover:text-indigo-600 rounded-xl hover:bg-indigo-50 border border-transparent hover:border-indigo-100/50 transition-all shadow-none hover:shadow-sm" title={t("yenile")}>
            <RefreshCw className="h-4.5 w-4.5" />
          </button>
          {unreadCount > 0 && <button type="button" onClick={markAllAsRead} className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200/80 bg-white px-3.5 py-2 text-xs font-bold text-slate-700 shadow-sm transition hover:bg-slate-50 hover:border-slate-300 active:bg-slate-100" title={t("hepsini_okundu_olarak_i_aretle")}>
