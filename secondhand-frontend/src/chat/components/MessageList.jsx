...
const MessageBubble = memo(({ message, isOwnMessage, onDeleteMessage }) => {
  ...
  return (
    <div className={`flex ${isOwnMessage ? 'justify-end' : 'justify-start'} mb-3`}>
      <div
        className={`max-w-xs lg:max-w-md px-4 py-3 rounded-2xl relative group shadow-sm transition-all duration-300 ease-in-out ${
          isOwnMessage
...
const MessageList = memo(({ 
  ...
  return (
    <div ref={messagesContainerRef} className="flex-1 min-h-0 overflow-y-auto p-6 custom-scrollbar bg-slate-50/50">
      {isLoadingMessages ? (
        ...
        <div className="flex flex-col items-center justify-center h-full text-center">
          ...
          <h3 className="text-sm font-bold text-slate-900 mb-1 tracking-tight">No messages yet</h3>
          <p className="text-xs text-slate-500 tracking-tight">Send the first message!</p>
        </div>
      ) : (
