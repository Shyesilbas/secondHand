...
const countBadge = (n) => {
    ...
    if (v <= 0) return null;
    return (
        <span className="min-w-[1.25rem] rounded-full bg-slate-200/90 px-2 py-0.5 text-center text-[11px] font-bold tabular-nums text-slate-700">
            {v > 99 ? '99+' : v}
        </span>
    );
};

const HeaderInboxMenu = ({ isOpen, onToggle, onClose, emailCount, chatCount }) => {
    const { unreadCount } = useInAppNotificationsContext();
    const hubTotal = (Number(unreadCount) || 0) + (Number(emailCount) || 0) + (Number(chatCount) || 0);

    const rowClass =
        'flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-slate-700 transition-colors hover:bg-slate-50/80 hover:text-slate-900';
    ...
    return (
        <div className="relative">
            <button
                ...
                onClick={onToggle}
                title="Inbox"
                className="group relative rounded-xl p-2.5 text-slate-600 transition-all duration-300 ease-in-out hover:bg-slate-100/50 hover:text-slate-900"
            >
            ...
            {isOpen && (
                <HeaderDropdownPanel className="!right-0 !w-64">
                    <div className="border-b border-slate-100 px-3 pb-2 pt-1">
                        <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Inbox</p>
                    </div>
                    <nav className="flex flex-col gap-0.5 p-1.5">
                        <Link to={tabUrl(INBOX_TABS.EMAILS)} onClick={onClose} className={rowClass}>
                            <Mail className="h-4 w-4 shrink-0 text-slate-500" />
                            <span className="min-w-0 flex-1 font-medium">Mail</span>
                        ...
                        <Link to={tabUrl(INBOX_TABS.NOTIFICATIONS)} onClick={onClose} className={rowClass}>
                            ...
                            <span className="min-w-0 flex-1 font-medium">Notifications</span>
                        ...
                        <Link to={tabUrl(INBOX_TABS.CHAT)} onClick={onClose} className={rowClass}>
                            ...
                            <span className="min-w-0 flex-1 font-medium">Chat</span>
