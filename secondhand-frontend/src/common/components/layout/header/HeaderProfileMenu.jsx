  ...
  return <>
        ...
        {isOpen && <HeaderDropdownPanel className="z-[9999]">
                <div className="px-4 py-3 border-b border-slate-200/60">
                    ...
                    <Link to={ROUTES.PROFILE} onClick={onClose} className="text-sm font-semibold text-slate-900 truncate tracking-tight block hover:text-slate-600 transition-colors cursor-pointer">
                        {user?.name || 'User'}
                    </Link>
                </div>
                ...
                <button onClick={onLogout} type="button" className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-red-600 hover:bg-red-50/80 transition-all duration-300 ease-in-out rounded-xl mx-1 cursor-pointer">
