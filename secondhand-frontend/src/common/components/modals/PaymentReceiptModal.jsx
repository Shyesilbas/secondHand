...
const StatusBadge = ({
  ...
  if (status === PAYMENT_STATUSES.ESCROW) {
    return <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-50 text-amber-600 border border-amber-100">
        ...
        <span className="text-[10px] font-bold uppercase tracking-tight">{t("in_escrow")}</span>
      </div>;
  }
  if (isSuccess) {
    return <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 text-emerald-600 border border-emerald-100">
        ...
        <span className="text-[10px] font-bold uppercase tracking-tight">{t("confirmed")}</span>
      </div>;
  }
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-slate-900/40 backdrop-blur-sm transition-all print:bg-white print:p-0">
        ...
        y: 20
          ...
          <div className="p-5 pb-2 flex items-center justify-between bg-white z-10">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-lg bg-slate-900 flex items-center justify-center text-white font-black text-[10px]">{t("s")}</div>
              <span className="text-[10px] font-black text-slate-900 uppercase tracking-widest">{t("digital_invoice")}</span>
            </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            <div className="p-6 pt-2 text-center border-b border-dashed border-slate-100 relative bg-white">
              ...
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2">{t("total_amount")}</p>
            ...
            <div className="p-6 space-y-6 bg-[#fafafa]/50">
              ...
              <div className="px-1">
                <div className="flex items-center gap-3 p-4 bg-indigo-600 rounded-2xl text-white shadow-lg shadow-indigo-200">
                  ...
                  <div>
                    <p className="text-[9px] font-black uppercase tracking-widest opacity-70">{t("transaction_category")}</p>
              ...
              {/* Details Section */}
              <div>
                <h3 className="text-[9px] font-black text-slate-300 uppercase tracking-[0.2em] mb-3 px-1">{t("receipt_details")}</h3>
              ...
              {/* Parties Section */}
              <div>
                <h3 className="text-[9px] font-black text-slate-300 uppercase tracking-[0.2em] mb-3 px-1">{t("parties")}</h3>
                <div className="grid grid-cols-2 gap-2.5">
                  <div className="p-3.5 bg-white rounded-2xl border border-slate-100/50 shadow-sm">
                    <p className="text-[8px] font-black text-slate-400 uppercase mb-1.5">{t("sender")}</p>
                  ...
                  <div className="p-3.5 bg-white rounded-2xl border border-slate-100/50 shadow-sm">
                    <p className="text-[8px] font-black text-slate-400 uppercase mb-1.5">{t("recipient")}</p>
                    <div className="flex items-center gap-2 min-w-0">
                      <div className="w-5 h-5 rounded-md bg-slate-50 flex items-center justify-center shrink-0">
                        <UserIcon className="w-2.5 h-2.5 text-slate-400" />
                      </div>
              ...
              <div className="pt-2 flex flex-col items-center gap-2.5 text-center">
                <div className="flex items-center gap-1.5 py-1.5 px-3 bg-emerald-50 text-emerald-600 rounded-full border border-emerald-100">
                  ...
                  <span className="text-[8px] font-black uppercase tracking-widest">{t("verified_by_secondhand_pay")}</span>
                </div>
          ...
          <div className="p-5 bg-white border-t border-slate-100 flex gap-3 print:hidden shrink-0">
            <button onClick={onClose} className="flex-1 py-3 px-4 text-[10px] font-bold uppercase tracking-widest rounded-xl bg-slate-50 text-slate-600 hover:bg-slate-100 transition-all active:scale-95">{t("back")}</button>
            <button onClick={() => window.print()} className="flex-1 flex items-center justify-center gap-2 py-3 px-4 text-[10px] font-bold uppercase tracking-widest rounded-xl bg-slate-900 text-white hover:bg-slate-800 shadow-lg shadow-slate-900/10 transition-all active:scale-95">
