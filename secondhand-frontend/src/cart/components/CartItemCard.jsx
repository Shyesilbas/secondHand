...
const CartItemCard = ({
  ...
  const unitPrice =
    parseFloat(hasCampaign ? item.listing.campaignPrice : item.listing.price) || 0;
  ...
  const { timeRemaining, isExpired, isReserved } = useReservationTimer(
    item.reservedAt,
  ...
  const metaParts = [
    item.listing.type,
    item.listing.city,
    [sellerName, sellerSurname].filter(Boolean).join(' ') || null,
  ].filter(Boolean);

  return (
    <div className="group px-4 py-4 transition-colors hover:bg-[#eef4fb] sm:px-5">
        ...
        <div className="relative shrink-0">
          {item.listing.imageUrl ? (
            <img
              ...
              alt=""
              loading="lazy"
              className={`h-20 w-20 border object-cover sm:h-24 sm:w-24 ${CART_SHAPE.radiusThumb}`}
          ...
          <div
            className={`flex h-20 w-20 items-center justify-center border bg-[#f7f6f5] sm:h-24 sm:w-24 ${CART_SHAPE.radiusThumb} ${
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
                ...
                <p className="mt-1 text-xs text-[#5f5b57]">
                  ...
                  {sellerTotalReviews > 0 && (
                    <span className="whitespace-nowrap">
                      ...
                      ·{' '}
                      <Star
                        className="inline-block h-3 w-3 align-text-bottom text-[#ca5010]"
                        fill="currentColor"
                        strokeWidth={0}
                        aria-hidden
                      />{' '}
            ...
            <button
              ...
              disabled={isRemoving}
              className="shrink-0 rounded-xl p-1.5 text-[#9c9894] transition hover:bg-[#eef4fb] hover:text-[#1466c6] disabled:opacity-40"
              title="Remove"
            >
          ...
          <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] text-[#5f5b57]">
            {isLowStock && (
              <span className="tabular-nums">Stock: {Number(maxStock)}</span>
            )}
            {isReserved && !isExpired && (
              <span
                className={`inline-flex items-center gap-1 tabular-nums ${
                  isReservationExpiring ? 'text-[#d13438]' : 'text-[#1466c6]'
                }`}
              >
            ...
            {isReserved && isExpired && (
              <span className="text-[#9c9894]">Expired</span>
            )}
            {hasCampaign && (
            ...
            {totalReviews > 0 && sellerTotalReviews === 0 && (
              <span className="tabular-nums">
                Rating {averageRating.toFixed(1)} ({totalReviews})
              </span>
            )}
          </div>
          ...
          <div className="mt-3 flex flex-wrap items-end justify-between gap-3">
            <div className="min-w-0">
              ...
              {(item.quantity > 1 || hasCampaign) && (
                <p className="mt-0.5 text-xs tabular-nums text-[#5f5b57]">
                  ...
                  {hasCampaign && (
                    <span className={item.quantity > 1 ? ' ml-2' : ''}>
            ...
            <div
              className={`inline-flex items-stretch overflow-hidden border bg-white ${CART_SHAPE.radiusBox}`}
              style={{ borderColor: CART_UI.border }}
            >
              <button
                type="button"
                ...
                className="px-2 py-1.5 text-[#5f5b57] transition enabled:hover:bg-[#f7f6f5] disabled:opacity-35"
              >
                <Minus className="h-3.5 w-3.5" strokeWidth={2} />
              </button>
              <span className="flex min-w-[2rem] items-center justify-center border-x px-2 text-sm font-medium tabular-nums text-[#1a1918]" style={{ borderColor: CART_UI.border }}>
                {item.quantity}
              </span>
              <button
                type="button"
          ...
          {item.notes && (
            <div
              className="mt-3 border-l-2 border-[#5f5b57] bg-[#f7f6f5] px-3 py-2 text-xs text-[#5f5b57]"
            >
              <span className="font-medium text-[#1a1918]">Note: </span>
              {item.notes}
            </div>
          )}
        </div>
