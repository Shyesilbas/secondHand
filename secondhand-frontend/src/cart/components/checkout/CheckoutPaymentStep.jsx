...
const CheckoutPaymentStep = ({
  ...
  return <div className="p-5 sm:p-7">
      ...
      <div className="mb-5">
        <label className="flex items-center gap-4 rounded-xl border border-transparent bg-indigo-50/15 p-5 ring-2 ring-primary shadow-sm">
        ...
        <div className="mt-3 space-y-2 pl-9 text-sm">
          <div className="flex justify-between gap-4">
            <span className="text-[#555]">{t("available")}</span>
            <span className="font-medium tabular-nums text-[#111]">{formatCurrency(walletBalance, cur)}</span>
          </div>
          <div className="flex justify-between gap-4">
            <span className="text-[#555]">{t("order_total")}</span>
            <span className="font-medium tabular-nums text-[#111]">{formatCurrency(totalAmount, cur)}</span>
          </div>
          {!hasEnoughBalance && <div className="mt-2 rounded-lg border border-[#f5d5d5] bg-[#fdf7f7] px-3 py-2.5 text-xs text-[#a4262c]">{t("insufficient_wallet_balance_add_funds_to")}</div>}
        </div>
      </div>
