    ...
    <div className="p-5 sm:p-7">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-2">
                        ...
                        {address.mainAddress && (
                          <span className="rounded-md border border-slate-200 bg-slate-50 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-slate-500">
                            Default
                          </span>
                        )}
                      </div>
        ...
        <div className="mb-6 rounded-xl border border-slate-100 bg-white shadow-sm p-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <label className="flex cursor-pointer select-none items-center gap-3">
              ...
              />
              <span className="text-sm font-semibold text-slate-800">Billing same as shipping</span>
            </label>
            ...
            {!billingSameAsShipping && (
              <div className="relative w-full sm:w-64">
                <select
                  ...
                  className="w-full appearance-none rounded-xl border border-slate-100 bg-slate-50/50 px-4 py-2.5 pr-10 text-sm text-slate-900 outline-none transition focus:border-indigo-600 focus:bg-white focus:ring-4 focus:ring-indigo-100 shadow-inner"
                >
      ...
      <div className="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          <label className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-slate-500">
            Order name <span className="font-normal text-slate-400 capitalize">(optional)</span>
          </label>
          <input
            type="text"
            className="w-full rounded-xl border border-slate-100 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition shadow-sm placeholder:text-slate-300 focus:border-indigo-600 focus:ring-4 focus:ring-indigo-100"
        ...
        <div>
          <label className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-slate-500">
            Notes <span className="font-normal text-slate-400 capitalize">(optional)</span>
          </label>
          <input
            type="text"
            className="w-full rounded-xl border border-slate-100 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition shadow-sm placeholder:text-slate-300 focus:border-indigo-600 focus:ring-4 focus:ring-indigo-100"
      ...
      <div className="hidden items-center justify-between border-t border-slate-100 pt-6 sm:flex">
        ...
        <div className="flex items-center gap-4">
          {warning && (
            <span className="text-xs text-amber-600 font-semibold tracking-wide">{warning}</span>
          )}
          <button
            type="button"
            onClick={onNext}
      ...
      <div className="sticky bottom-0 -mx-5 mt-6 flex flex-col gap-2 border-t border-slate-100 bg-white px-5 py-4 sm:hidden">
        {warning && (
          <p className="text-center text-xs text-amber-600 font-semibold tracking-wide mb-1">{warning}</p>
        )}
        <div className="grid grid-cols-2 gap-2">
          ...
          <button
            ...
            onClick={onNext}
            disabled={!isStepValid}
            className="flex items-center justify-center gap-1.5 rounded-xl bg-slate-900 py-3.5 text-xs font-bold uppercase tracking-wider text-white transition-all hover:bg-black disabled:bg-slate-100 disabled:text-slate-400"
          >
            Continue
            <ArrowRight className="h-4 w-4" strokeWidth={2} />
          </button>
