    ...
    if (e.target === e.currentTarget) onClose?.();
      ...
      <div className="w-full max-w-4xl bg-white rounded-3xl shadow-2xl border border-[#e5e3df] overflow-hidden animate-in fade-in zoom-in duration-200 flex flex-col max-h-[90vh]">
        ...
        <div className="px-8 py-5 border-b border-[#f0efed] bg-[#fafaf9] flex items-center justify-between shrink-0">
          ...
          <button type="button" onClick={onClose} className="p-2.5 rounded-full hover:bg-[#ebeae8] text-[#9c9894] transition-colors">
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          ...
          <div className="w-72 bg-[#fafaf9] border-r border-[#f0efed] p-6 space-y-4 overflow-y-auto shrink-0">
            <div className="text-xs font-bold text-[#9c9894] uppercase tracking-widest mb-4">{t("choose_type")}</div>
            
            <button onClick={() => setForm(p => ({
            ...p,
            campaignType: 'STANDARD',
            minQuantity: 1
          }))} className={`w-full text-left p-4 rounded-2xl border-2 transition-all ${form.campaignType === 'STANDARD' ? 'bg-white border-indigo-600 shadow-md' : 'bg-transparent border-transparent hover:bg-[#ebeae8] text-[#5f5b57]'}`}>
            ...
            mode: 'ALL',
            minQuantity: 2
          }))} className={`w-full text-left p-4 rounded-2xl border-2 transition-all ${form.campaignType === 'BUNDLE' ? 'bg-white border-indigo-600 shadow-md' : 'bg-transparent border-transparent hover:bg-[#ebeae8] text-[#5f5b57]'}`}>
          ...
          <div className="flex-1 overflow-y-auto p-8 bg-white custom-scrollbar">
            <div className="space-y-8">
              ...
              <section className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div className="relative">
                    ...
                    ...p,
                    name: e.target.value
                  }))} className="w-full px-4 py-3.5 border border-[#e5e3df] rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-600/20 focus:border-indigo-600 transition-all" placeholder={t("e_g_summer_special_2024")} />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  }))} type="number" className="w-full px-4 py-3.5 border border-[#e5e3df] rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-600/20 focus:border-indigo-600 transition-all" placeholder="0.00" />
                  </div>
                </div>
              ...
              {form.campaignType === 'BUNDLE' ? <section className="p-6 bg-indigo-50 border border-indigo-100 rounded-3xl space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-sm font-bold text-indigo-900">{t("bundle_configuration")}</div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <p className="text-xs text-indigo-700 leading-relaxed">{t("this_bundle_will_automatically_apply_to")}<b>{t("all_your_active_listings")}</b>{t("customers_will_receive_the_discount_only")}<b>{form.minQuantity || 2}{t("or_more")}</b>{t("items_to_their_cart")}</p>
                  </div>
                  ...
                  eligibleListingIds: [],
                  eligibleTypes: []
                }))} className={`px-4 py-2 rounded-full text-xs font-bold border transition-all ${form.mode === m ? 'bg-[#1a1918] text-white border-[#1a1918]' : 'bg-white text-[#5f5b57] border-[#e5e3df] hover:border-[#1a1918]'}`}>
                ...
                }} className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all ${form.eligibleTypes.includes(t) ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-[#fafaf9] text-[#5f5b57] border-[#e5e3df]'}`}>
                          {t}
                        </button>)}
                    </div>}
                </section>}
              ...
              <section className="flex items-center justify-between p-4 bg-[#fafaf9] rounded-2xl border border-[#f0efed]">
                <div>
                  ...
                  <div className="text-xs text-[#9c9894]">{t("campaign_will_be_visible_once_saved")}</div>
                </div>
        ...
        <div className="px-8 py-5 border-t border-[#f0efed] bg-[#fafaf9] flex items-center justify-end gap-4 shrink-0">
          <button type="button" onClick={onClose} className="px-6 py-2.5 text-sm font-bold text-[#5f5b57] hover:text-[#1a1918] transition-colors">{t("cancel")}</button>
