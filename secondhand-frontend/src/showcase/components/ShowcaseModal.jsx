  ...
  const renderStepContent = useCallback(() => {
    if (step === 1) {
      ...
      return <div className="space-y-6">
                    {/* Duration picker */}
                    <div>
                        <p className="text-[12px] font-semibold text-slate-500 uppercase tracking-widest mb-3">{t("choose_duration")}</p>
                        <div className="grid grid-cols-4 gap-2 mb-4">
                            {durationOptions.map(opt => <button key={opt} type="button" onClick={() => setDays(opt)} className={`py-3 rounded-xl text-[13px] font-bold transition-all duration-200 ${days === opt ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/25 scale-105' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                                    {opt}{t("d")}</button>)}
                        </div>
                        <div className="flex items-center gap-3 bg-slate-50 rounded-xl px-4 py-3 border border-slate-200">
                            <span className="text-[12px] text-slate-500 font-medium shrink-0">{t("custom_days")}</span>
                            <input type="number" min="1" max="30" value={days} onChange={handleDaysChange} className="flex-1 bg-transparent text-[14px] font-bold text-slate-800 focus:outline-none w-0" />
                            {showcasePricing && <span className="text-[11px] text-slate-400 shrink-0">
                    ...
                    {showcasePricing && <div className="rounded-2xl border border-slate-200 overflow-hidden">
                            <div className="bg-slate-50 px-4 py-3 border-b border-slate-200">
                                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">{t("price_summary")}</p>
                            </div>
                            <div className="px-4 py-4 space-y-2.5">
                                ...
                                <div className="pt-3 border-t border-slate-100 flex justify-between items-baseline">
                                    <span className="text-[13px] font-bold text-slate-700">{t("total")}</span>
    ...
    if (isListingLoading) {
      return <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md mx-4 p-10 text-center">
                        ...
                        <p className="text-[13px] text-slate-500 font-medium">{t("loading_listing")}</p>
                    </div>
                </div>;
    }
    if (!listingId || listingError || !listing) {
      return <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm mx-4 p-8 text-center">
                        ...
                        <h2 className="text-[15px] font-bold text-slate-800 mb-2">{t("something_went_wrong")}</h2>
                        <p className="text-[13px] text-slate-400 mb-6">{listingError || 'Listing not found.'}</p>
                        <button className="px-6 py-2.5 bg-slate-900 text-white rounded-xl text-[13px] font-semibold hover:bg-slate-800 transition-colors" onClick={onClose}>{t("close")}</button>
                    </div>
                </div>;
    }
    return <>
                ...
                return <div key={id} className="flex flex-col items-center gap-1.5 z-10">
                        ...
                        <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between bg-slate-50/60">
                            <button className="px-4 py-2 text-[13px] font-semibold text-slate-500 hover:text-slate-800 transition-colors rounded-xl hover:bg-slate-100" onClick={handlePrevStep}>
                                {step > 1 ? '← Back' : 'Cancel'}
                            </button>
                            {step < 3 && <button className="px-6 py-2.5 bg-indigo-600 text-white rounded-xl text-[13px] font-bold hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-500/25 disabled:opacity-40 disabled:cursor-not-allowed disabled:shadow-none" onClick={handleNextStep} disabled={step === 1 && (days < 1 || days > 30) || step === 2 && !allAgreementsAccepted}>
