...
const ShowcasePayment = forwardRef(function ShowcasePayment({
  ...
  useOtpSuggestedToast({ suggestedCode: suggestedFromInbox, enabled: step === 3 });
  ...
  const btnPrimaryClass = embedded
    ...
    : 'w-full py-2.5 rounded-xl text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 disabled:opacity-45 transition-colors';
      ...
      case 1:
        return (
          <div className={embedded ? 'space-y-4' : 'space-y-3'}>
            <div>
              <h3 className={`${headingClass} ${embedded ? 'mb-1' : 'mb-4 text-center'}`}>
                Payment method
              </h3>
              {embedded ? (
        ...
        return (
          <div className={embedded ? 'space-y-4' : 'space-y-3'}>
            <div>
              ...
              {embedded ? (
                <p className={mutedClass}>
                  Confirm the total, then send the verification code.
                </p>
              ) : null}
            </div>
            <div className={`${embedded ? 'rounded-2xl border border-slate-200 bg-slate-50/80 p-4 space-y-2' : 'space-y-2 mb-4'}`}>
              <div className="flex justify-between gap-3 text-[13px]">
                <span className={`${embedded ? 'text-slate-500' : 'font-medium'} shrink-0`}>Listing</span>
              ...
              <div className={`flex justify-between ${embedded ? 'text-[13px] text-slate-600' : ''}`}>
                <span className={embedded ? 'text-slate-500' : 'font-medium'}>Duration</span>
                <span>{days} days</span>
              </div>
              <div className={`flex justify-between ${embedded ? 'text-[13px] text-slate-600' : ''}`}>
            ...
            {showcasePricing ? (
              <div className={`${embedded ? 'rounded-2xl border border-slate-200 p-4 space-y-2' : 'p-3 bg-gray-50 rounded-lg space-y-2'}`}>
              ...
              <div className={`flex justify-between ${embedded ? 'rounded-2xl border border-slate-200 p-4' : 'p-3 bg-gray-50 rounded'}`}>
                <span className="font-bold text-lg">Total:</span>
                <span className={`text-xl font-bold ${embedded ? 'text-indigo-600' : 'text-emerald-600'} tabular-nums`}>{totalCost}₺</span>
              </div>
            )}
            {error ? (
              <div className="p-3 rounded-xl border border-rose-200 bg-rose-50 text-[13px] text-rose-800">
                {error}
              </div>
            ) : null}
        ...
        return (
          <div className="space-y-4">
            <div className="flex gap-3 items-start">
              {embedded ? (
                <div className="w-10 h-10 shrink-0 rounded-xl bg-indigo-600 flex items-center justify-center shadow-md shadow-indigo-500/25">
              ...
              <div className={`min-w-0 flex-1 ${embedded ? '' : 'text-center'}`}>
                ...
                <p className={`${mutedClass} mt-1 leading-relaxed`}>
                  Enter the{' '}
                  <span className="font-semibold text-slate-700">{OTP_CODE_LENGTH}</span>
                  -digit code we emailed you to{' '}
                  <span className="font-semibold text-slate-700">finish payment</span>
                  .
                </p>
                {otpTtlActive ? (
                  <p
                    className={`text-[11px] font-semibold mt-2 tabular-nums ${
                      otpTtlExpired ? 'text-amber-700' : embedded ? 'text-slate-600' : 'text-gray-600'
                    }`}
                  >
                    {otpTtlExpired
                      ? 'Code expired — go back and send a new code.'
                      : `Expires in: ${otpTtlFormatted}`}
                  </p>
                ) : null}
              </div>
            </div>
            ...
            {suggestedFromInbox ? (
              <OtpSuggestionBanner suggestedCode={suggestedFromInbox} onApply={setVerificationCode} />
            ) : null}
            ...
            <button
              type="button"
              className={`${btnPrimaryClass} inline-flex items-center justify-center gap-2`}
            ...
            >
              {loading ? (
                ...
                <>
                  {embedded ? <ShieldCheck className="w-4 h-4" /> : null}
                  Finish payment
                </>
              )}
            </button>
    ...
    <div>
      <div className="flex items-center gap-2 mb-6">
        {[1, 2, 3].map((s) => (
          <div
            key={s}
            className={`h-1.5 flex-1 rounded-full ${step >= s ? 'bg-emerald-500' : 'bg-gray-200'}`}
          />
        ))}
      </div>
      {renderStepBody()}
      {step !== 3 ? (
        <div className="flex items-center justify-between mt-6">
          <button
            type="button"
            className="px-4 py-2 border rounded-lg text-sm font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-45"
            onClick={() => innerBack()}
            disabled={loading}
          >
            {step > 1 ? 'Back' : 'Cancel'}
          ...
          {step === 1 ? (
            <button
              type="button"
              className="px-5 py-2 rounded-lg text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 disabled:opacity-45"
              onClick={() => setStep(2)}
              disabled={!canContinuePayment || loading}
            >
              Continue
