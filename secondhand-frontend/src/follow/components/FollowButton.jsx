...
const FollowButton = memo(({
  ...
  return <div className={`relative ${className}`} ref={dropdownRef}>
            <button onClick={handleMainClick} disabled={isToggling} className={`
                    ...
                    ${isFollowing ? 'bg-secondary-100 text-text-primary hover:bg-secondary-200 border border-border-light' : 'bg-accent-indigo-600 text-white hover:bg-accent-indigo-700 shadow-sm'}
            ...
            {isFollowing && showDropdown && isDropdownOpen && <div className="absolute right-0 mt-2 w-48 bg-background-primary rounded-lg shadow-lg border border-border-light py-1 z-[100]">
                    <button onClick={handleToggleNotifications} disabled={isTogglingNotifications} className="w-full px-4 py-2.5 text-left text-sm text-text-primary hover:bg-secondary-50 flex items-center gap-2.5 transition-colors">
                        {isTogglingNotifications ? <Loader2 className="w-4 h-4 animate-spin" /> : notifyOnNewListing ? <Bell className="w-4 h-4 text-accent-indigo-600" /> : <BellOff className="w-4 h-4 text-text-muted" />}
                        <span>{notifyOnNewListing ? 'Notifications On' : 'Notifications Off'}</span>
                    </button>
                    <div className="border-t border-border-light my-1" />
                    <button onClick={handleUnfollow} className="w-full px-4 py-2.5 text-left text-sm text-status-error-DEFAULT hover:bg-status-error-bg flex items-center gap-2.5 transition-colors">
