  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={handleBackdropClick}>
            <div className="bg-white rounded-2xl w-full max-w-md mx-4 shadow-2xl overflow-hidden">
                ...
                <div className="flex border-b border-gray-100">
                    <button onClick={() => setActiveTab('followers')} className={`flex-1 py-3 text-sm font-medium transition-colors relative ${activeTab === 'followers' ? 'text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}>{t("followers")}{followersData && <span className="ml-1.5 text-xs text-gray-400">
                                ({followersData.totalElements || 0})
                            </span>}
                        {activeTab === 'followers' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-indigo-600" />}
                    </button>
                    <button onClick={() => setActiveTab('following')} className={`flex-1 py-3 text-sm font-medium transition-colors relative ${activeTab === 'following' ? 'text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}>{t("following")}{followingData && <span className="ml-1.5 text-xs text-gray-400">
                ...
                <div className="max-h-[400px] overflow-y-auto">
                    {isLoading ? <div className="flex items-center justify-center py-12">
                        ...
                        </div> : list.length === 0 ? <div className="flex flex-col items-center justify-center py-12 text-gray-500">
            ...
            return <li key={item.id}>
                                        <Link to={ROUTES.USER_PROFILE(personId)} onClick={onClose} className="flex items-center gap-3 px-6 py-3 hover:bg-gray-50 transition-colors">
