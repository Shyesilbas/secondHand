    ...
    >
      <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
          <div>
            <div className="text-lg font-semibold text-gray-900">Counter Offer</div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-800 mb-1">Total Price</label>
              <input
                type="number"
                min={0}
                step="0.01"
          ...
          <div className="rounded-xl border border-gray-200 bg-gray-50 px-4 py-3">
            <div className="text-xs text-gray-600">
              Countering for <span className="font-semibold text-gray-900">{Number(quantity) || 1}</span> item(s).
              {previewTotal != null && (
                <>
                  {' '}Total: <span className="font-semibold text-gray-900">{formatCurrency(previewTotal, currency)}</span>
                </>
              )}
            </div>
          </div>
