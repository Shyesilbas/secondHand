...
const OfferTrackingCard = ({
  ...
  return <article className={`group relative overflow-hidden rounded-2xl border bg-white shadow-sm transition-all hover:shadow-md ${needsAttention ? 'border-amber-300/90 ring-1 ring-amber-200/50' : 'border-slate-200/80'} ${isExpired ? 'opacity-75 grayscale-[0.35]' : ''}`}>
      ...
      <div className="flex flex-col gap-4 p-4 sm:flex-row sm:gap-5 sm:p-5">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              {o.listingId ? <button type="button" onClick={() => onOpenListing(o.listingId)} className="text-left text-base font-semibold leading-snug text-slate-900 transition hover:text-indigo-600">
                ...
                </button> : <p className="text-base font-semibold text-slate-900 line-clamp-2">
        ...
        <div className="flex shrink-0 flex-row flex-wrap items-center justify-end gap-2 border-t border-slate-100 pt-4 sm:w-44 sm:flex-col sm:border-t-0 sm:pt-0">
          {isExpired ? <span className="text-xs font-medium text-slate-500">{t("offer_expired")}</span> : canCheckout ? <button type="button" onClick={() => onCheckout(o.id)} className="inline-flex w-full min-w-[8.5rem] items-center justify-center rounded-xl bg-indigo-600 px-3 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-indigo-700">
              <ShoppingCart className="mr-1.5 h-4 w-4" />{t("checkout")}</button> : canActAsSeller ? <>
              <button type="button" onClick={() => onAccept(o.id)} className="inline-flex w-full min-w-[8.5rem] items-center justify-center rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-800 transition hover:bg-emerald-100">
