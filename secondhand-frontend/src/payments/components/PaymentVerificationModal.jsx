      ...
      <div className={`border rounded-lg p-4 ${hasSufficientBalance ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <div>
              <p className="font-medium text-gray-900">{t("ewallet")}</p>
              <p className="text-sm text-gray-600">{t("digital_wallet")}</p>
            </div>
          </div>
        ...
        {!hasSufficientBalance && <div className="mt-3 p-2 bg-red-100 rounded border border-red-200">
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xl">
      <div className="w-full max-w-3xl mx-4 overflow-hidden rounded-[2.5rem] bg-white shadow-[0_24px_80px_rgba(15,23,42,0.38)] animate-in fade-in zoom-in-95">
          ...
          {step === VERIFICATION_STEPS.REVIEW ? <div className="mb-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <button type="button" onClick={handleResend} disabled={isResendingCode} className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-60">
        ...
        <div className="flex items-center gap-3 border-t border-slate-100 bg-slate-50/80 px-6 py-4">
          <button onClick={handleClose} className="rounded-xl border border-slate-200 px-4 py-2 text-sm text-slate-600 hover:bg-white">{t("cancel")}</button>
          {step === VERIFICATION_STEPS.REVIEW ? <button onClick={handleStart} disabled={!canStartPayment || isProcessing} className="ml-auto rounded-2xl bg-slate-900 px-4 py-2 text-sm font-semibold tracking-tight text-white shadow-md hover:bg-slate-800 disabled:opacity-60">
            ...
            </button> : <button onClick={handleVerify} disabled={isProcessing || (verificationCode || '').length !== OTP_CODE_LENGTH || otpTtlExpired} className="ml-auto rounded-2xl bg-emerald-600 px-4 py-2 text-sm font-semibold tracking-tight text-white shadow-md hover:bg-emerald-700 disabled:opacity-60">
