...
const EWalletPaymentMethod = ({ eWallet, feeConfig }) => {
  ...
  return (
    <div className="space-y-3">
      <div className={`border rounded-2xl p-4 transition-all ${hasSufficientBalance ? 'bg-emerald-50/60 border-emerald-200' : 'bg-amber-50/60 border-amber-200'}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold ${hasSufficientBalance ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
              ₺
            </div>
            <div>
          ...
          <div className="text-right">
            ...
            <span className={`text-xs font-semibold ${hasSufficientBalance ? 'text-emerald-700' : 'text-amber-700'}`}>
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="rounded-2xl border border-emerald-100 bg-emerald-50/50 p-4 flex items-center justify-between">
              <div className="min-w-0 pr-3">
                <p className="text-[10px] font-bold text-emerald-700 uppercase tracking-widest">{t("listing")}</p>
                <p className="text-sm font-bold text-slate-900 truncate mt-0.5">{selectedListing?.title}</p>
              </div>
              <div className="text-right flex-shrink-0">
                <p className="text-[10px] font-bold text-emerald-700 uppercase tracking-widest">{t("amount")}</p>
                  ...
                  <div className="flex items-center justify-between pt-2">
                    ...
                    {otpTtlActive && (
                      <div className={`text-xs font-medium tabular-nums ${otpTtlExpired ? 'text-amber-700' : 'text-slate-500'}`}>
                        {otpTtlExpired ? 'Code expired' : <>{t("expires_in")}: <span className="font-mono font-bold text-slate-700">{otpTtlFormatted}</span></>}
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex items-center justify-end gap-3">
            ...
            {step === VERIFICATION_STEPS.REVIEW ? (
              <button
                ...
                className="px-5 py-2.5 rounded-xl bg-emerald-600 text-xs font-extrabold text-white shadow-lg shadow-emerald-600/20 hover:bg-emerald-700 active:translate-y-0 disabled:opacity-50 transition-all"
              >
              ...
              <button
                ...
                className="px-5 py-2.5 rounded-xl bg-emerald-600 text-xs font-extrabold text-white shadow-lg shadow-emerald-600/20 hover:bg-emerald-700 disabled:opacity-50 transition-all"
              >
