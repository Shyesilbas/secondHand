...
const PaymentAgreementsSection = ({
  ...
  if (loading) {
    return <div className="flex justify-center items-center py-4 text-sm text-gray-500">
  ...
  return <div className="space-y-4">
      ...
      {agreements.map(agreement => <div key={agreement.agreementId} className="flex items-start space-x-3">
          <div className="flex items-center h-5">
            <input id={`agreement-${agreement.agreementId}`} name={`agreement-${agreement.agreementId}`} type="checkbox" checked={acceptedAgreements.has(agreement.agreementId)} onChange={() => handleAgreementToggle(agreement.agreementId)} className="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-2 focus:ring-indigo-500/20" />
          </div>
          <div className="min-w-0 flex-1 text-sm">
            <label htmlFor={`agreement-${agreement.agreementId}`} className="font-medium text-gray-900 cursor-pointer">
              {AGREEMENT_TYPE_LABELS[agreement.agreementType] || agreement.agreementType}
            </label>
            <p className="text-gray-500">
              <button type="button" onClick={() => handleAgreementClick(agreement)} className="text-indigo-600 hover:text-indigo-500 underline">{t("read_full_agreement")}</button>
            </p>
          </div>
        </div>)}

      {(error || !areAllAgreementsAccepted() && agreements.length > 0) && <div className="p-3 bg-red-50 border border-red-200 rounded text-red-600 text-sm">
