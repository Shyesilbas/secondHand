...
const DraftListingsList = ({
  ...
  return <div className="lg:col-span-2">
            <div className="rounded-3xl border border-slate-200/60 bg-white/80 px-5 py-5 shadow-[0_18px_45px_rgba(15,23,42,0.04)]">
                ...
                {listings.map(listing => <div key={listing.id} className={`rounded-2xl border p-5 cursor-pointer transition-all ${selectedListing?.id === listing.id ? 'border-indigo-500 ring-2 ring-indigo-500 shadow-lg shadow-indigo-100/50' : 'border-slate-200 hover:border-slate-300'}`} onClick={() => onSelectListing(listing)}>
                        <div className="flex items-start justify-between">
                            <div className="flex-1">
                                ...
                                <div className="flex items-center space-x-4 text-sm text-slate-500">
                                    ...
                                    <span>•</span>
                                    <span className="rounded-full bg-amber-50 px-2 py-0.5 text-xs font-medium text-amber-700">{t("draft")}</span>
                                </div>
                            </div>
                            
                            <div className="ml-4 flex flex-col items-end gap-2">
                                <div className={`flex h-6 w-6 items-center justify-center rounded-full border-2 ${selectedListing?.id === listing.id ? 'border-indigo-500 bg-indigo-500' : 'border-slate-300'}`}>
                                    {selectedListing?.id === listing.id && <svg className="h-4 w-4 text-white" fill="currentColor" viewBox="0 0 20 20">
