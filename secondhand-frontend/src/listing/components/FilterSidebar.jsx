      ...
      `} aria-labelledby="listing-filters-title">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-3 py-4">
          ...
          <div>
              <div className="mb-3 flex gap-1 rounded-xl bg-slate-200/70 p-1">
                  {['category', 'status'].map(tab => <button key={tab} type="button" onClick={() => setMineActiveTab(tab)} className={`flex-1 px-3 py-2.5 text-sm font-semibold rounded-lg transition-all capitalize ${mineActiveTab === tab ? 'bg-white text-slate-900 shadow-sm ring-1 ring-slate-200/70' : 'text-slate-500 hover:text-slate-700'}`}>
                      {tab}
                    </button>)}
                </div>
                ...
                {selectedCategory && <div className="flex items-center justify-between rounded-xl border border-slate-200/80 bg-white px-3 py-2.5 shadow-sm">
                    <span className="text-sm font-semibold tracking-tight text-slate-800">
                      {listingConfig?.label || selectedCategory}
                    </span>
                    <button type="button" onClick={() => onCategoryChange(null)} className="rounded-lg px-2 py-1 text-xs font-semibold text-indigo-600 transition-colors hover:bg-indigo-50">{t("change")}</button>
                  </div>}
                ...
                {selectedCategory && !filterConfig && <div className="rounded-xl border border-dashed border-slate-200 bg-white px-4 py-6 text-center">
        ...
        <footer className="flex-shrink-0 border-t border-slate-200/80 bg-white/95 px-4 py-3 backdrop-blur-sm">
          <div className="flex gap-2">
            {mode !== 'mine' && <button type="button" onClick={handleApplyFilters} className="flex-1 rounded-xl bg-indigo-600 px-4 py-3 text-sm font-semibold text-white shadow-md shadow-indigo-500/25 transition-colors hover:bg-indigo-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/45 focus-visible:ring-offset-2">{t("apply_filters")}</button>}
            <button type="button" onClick={handleReset} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-3 text-sm font-semibold text-slate-600 transition-colors hover:bg-slate-50 hover:border-slate-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/35">
...
const OptionRow = ({
  ...
  active,
  onClick
}) => <button type="button" onClick={onClick} className={`w-full rounded-xl px-3 py-2.5 text-left text-sm font-medium transition-colors ${active ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}>
    {label}
  </button>;
const CategoryChip = ({
  label,
