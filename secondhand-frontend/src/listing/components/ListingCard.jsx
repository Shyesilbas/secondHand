...
const ListingCard = memo(({ listing, onDeleted }) => {
    ...
    return (
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden hover:border-gray-300 hover:shadow-lg transition-all duration-200 group">
            {/* Header with badges */}
            <div className="relative p-5 pb-4">
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                        <span className={`inline-flex px-3 py-1.5 rounded-full text-xs font-semibold ${getStatusBadgeClass(listing.status)}`}>
                            {listing.status}
                        </span>
                        {isInShowcase && (
                            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs bg-gradient-to-r from-yellow-100 to-yellow-200 text-yellow-800 font-semibold">
                ...
                <Link to={ROUTES.LISTING_DETAIL(listing.id)} className="block group-hover:text-gray-900">
                    <div className="flex items-start justify-between mb-3">
                        ...
                        {listing.listingNo && (
                            <span className="text-xs text-gray-500 bg-gray-100 px-2.5 py-1.5 rounded-lg whitespace-nowrap ml-3 flex-shrink-0 font-medium">
                                #{listing.listingNo}
                            </span>
                        )}
                    </div>
                    ...
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <div className="flex items-center gap-1.5 text-gray-500 text-sm">
                            ...
                            <span className="truncate max-w-32 font-medium">{listing.district}, {listing.city}</span>
                        </div>
                    </div>
                </Link>
            </div>
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-3">
                            <div className="w-9 h-9 bg-gradient-to-br from-gray-300 to-gray-400 rounded-full flex items-center justify-center shadow-sm">
                                <span className="text-gray-700 font-semibold text-sm">{listing.sellerName?.[0]?.toUpperCase()}</span>
                            </div>
                            <span className="text-sm font-semibold text-gray-700 truncate max-w-24">
