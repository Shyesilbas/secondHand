...
const ListingWizard = ({
  ...
  const footerElement = <div className={`${t.bottomBar} shrink-0`}>
      <div className={uiKey === 'browse' ? 'w-full py-2' : 'mx-auto max-w-5xl px-4 py-3.5 sm:px-6 lg:px-8'}>
        ...
        <div className="flex items-center justify-between gap-3">
          <button type="button" onClick={currentStep === 1 ? onBack : onPrev} className={t.footerBackBtn}>
            <ChevronLeft className="h-4 w-4" />
            {currentStep === 1 ? 'Cancel' : 'Back'}
          </button>

          <div className="flex items-center gap-2.5">
            {onSaveDraft && isLastStep && <button type="button" onClick={onSaveDraft} disabled={isLoading || !canSubmit} className={`${t.draftBtnClass} disabled:cursor-not-allowed disabled:opacity-50`}>
                <Save className="h-3.5 w-3.5" />{t("save_draft")}</button>}

            {!isLastStep ? <motion.button whileHover={{
            scale: 1.02
          }} whileTap={{
            scale: 0.98
            ...
            e.preventDefault();
            onNext();
          }} disabled={!canSubmit} className={t.primaryBtn}>
                {continueLabel}
                <ChevronRight className="h-4 w-4" />
              </motion.button> : lastStepAction ? <motion.button whileHover={{
            scale: 1.02
          }} whileTap={{
            scale: 0.98
          }} type="button" onClick={e => {
          ...
          }} disabled={Boolean(lastStepAction.disabled)} className={t.primaryBtn}>
          ...
          }} disabled={isLoading || !canSubmit} className={t.primaryBtn}>
                {isLoading ? <>
  ...
  if (isComposerDock) {
    return <div className={shellClass}>
        <header className="shrink-0 border-b border-zinc-200/60 wizard-glass-elevated">
          <div className="mx-auto flex w-full max-w-lg flex-col gap-3 px-4 py-3.5 sm:px-5">
            ...
            <nav className="flex flex-wrap items-baseline gap-x-0.5 gap-y-1 text-caption text-zinc-500" aria-label={t("steps")}>
              ...
              const piece = canGoBack ? <button key={step.id} type="button" onClick={() => onStepPick(step.id)} className="max-w-[9rem] truncate text-left text-zinc-500 underline decoration-zinc-300/80 underline-offset-2 transition-colors hover:text-zinc-900 hover:decoration-zinc-500">
                    {label}
                  </button> : <span key={step.id} className={`max-w-[9rem] truncate ${isActive ? 'font-medium text-zinc-950' : done ? 'text-zinc-600' : 'text-zinc-400'}`}>
                    {label}
                  </span>;
              return <span key={step.id} className="inline-flex items-baseline gap-x-0.5">
            ...
            <div className="h-1 w-full overflow-hidden rounded-full bg-zinc-100">
              <div className="h-full rounded-full wizard-progress-fill transition-[width] duration-500 ease-out" style={{
              width: `${progress}%`
            }} />
            </div>
          </div>
        ...
        <div className="mx-auto flex w-full max-w-lg min-h-0 flex-1 flex-col px-4 sm:px-5">
          ...
          <div ref={stepPanelRef} className="min-h-0 flex-1 overflow-y-auto overscroll-y-contain pb-28 pt-2 [-webkit-overflow-scrolling:touch]">
            <AnimatePresence mode="wait" custom={direction}>
              <motion.div key={currentStep} custom={direction} variants={stepVariants} initial="enter" animate="center" exit="exit">
        ...
        <footer className={`${t.bottomBar} shrink-0 mt-auto pb-[max(1rem,env(safe-area-inset-bottom))]`}>
          <div className="mx-auto max-w-lg px-4 py-3 sm:px-5">
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                {onSaveDraft && isLastStep && <button type="button" onClick={onSaveDraft} disabled={isLoading || !canSubmit} className={`${t.draftBtnClass} disabled:cursor-not-allowed disabled:opacity-50`}>
                    <Save className="h-3 w-3" />{t("draft")}</button>}
                {!isLastStep ? <button type="button" onClick={e => {
                e.preventDefault();
                onNext();
              }} disabled={!canSubmit} className={t.primaryBtn}>
                    {continueLabel}
                    <ChevronRight className="h-3.5 w-3.5" />
                  </button> : lastStepAction ? <button type="button" onClick={e => {
                e.preventDefault();
                lastStepAction.onClick?.();
              }} disabled={Boolean(lastStepAction.disabled)} className={t.primaryBtn}>
                    {lastStepAction.label}
                ...
                if (onSubmit) onSubmit(e);
                    ...
                    {isLoading ? <>
                        <Loader2 className="h-3.5 w-3.5 animate-spin" />{t("publishing")}</> : <>
  ...
  return <div className={shellClass}>
      ...
      <div className={`${t.stickyOuter} shrink-0 wizard-glass-elevated`}>
          ...
          <div className="flex h-14 items-center justify-between">
            ...
            <div className="flex max-w-[min(60vw,18rem)] items-center gap-1 overflow-x-auto py-1 sm:max-w-none sm:gap-2">
              ...
              return <div key={step.id} className="flex shrink-0 items-center gap-1 sm:gap-1.5">
                    ...
                    {step.id < steps.length && <div className={`h-0.5 rounded-full transition-all duration-500 ease-out sm:w-6 ${isCompleted ? `${t.connectorDone} w-4` : `${t.connectorTodo} w-3`}`} />}
                  </div>;
            })}
            </div>

            <span className={`hidden tabular-nums text-xs font-medium sm:block ${t.stepMeta}`}>{t("step")}{currentStep}/{steps.length}
            </span>
          </div>
        </div>
        ...
        <div className={`h-1 w-full ${t.progressTrack}`}>
          <div className="h-full wizard-progress-fill rounded-r-full transition-all duration-500 ease-out" style={{
          width: `${progress}%`
        }} />
        </div>
      </div>
      ...
      <div className="mx-auto w-full max-w-5xl px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-12 lg:gap-12">
          ...
          <div className="hidden lg:col-span-3 lg:block">
            <nav className={layoutViewportLocked ? 'sticky top-20 space-y-1' : 'sticky top-24 space-y-1'} aria-label={t("form_steps")}>
              {steps.map(step => {
              const isActive = currentStep === step.id;
              const isCompleted = currentStep > step.id;
              const goBackHere = typeof onStepPick === 'function' && step.id < currentStep;
              const rowInner = <>
                    <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-xs font-bold transition-all duration-300 ${isActive ? t.sidebarStepActive : isCompleted ? t.sidebarStepDone : t.sidebarStepTodo}`}>
                      {isCompleted ? <span className="wizard-check-bounce inline-flex">
                          <Check className="h-3.5 w-3.5" />
                        </span> : step.id}
                    </div>
                    <div className="min-w-0 text-left">
                      <p className={`text-xs font-semibold transition-colors duration-200 ${isActive ? t.sidebarTitleActive : isCompleted ? t.sidebarTitleDone : t.sidebarTitleTodo}`}>
                        {step.title}
                      </p>
                      {step.description && isActive ? <p className={`mt-0.5 line-clamp-2 ${t.sidebarStepDesc}`}>{step.description}</p> : null}
                    </div>
                  </>;
              if (goBackHere) {
                return <button key={step.id} type="button" onClick={() => onStepPick(step.id)} className="relative flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left transition-all duration-200 hover:bg-black/[0.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-400/40">
                      {rowInner}
                    </button>;
              }
              return <div key={step.id} className={`relative flex items-center gap-3 rounded-xl px-3 py-2.5 transition-all duration-300 ${isActive ? 'bg-zinc-50/80 shadow-sm border border-zinc-200/50' : ''}`}>
                    {/* Active bar indicator */}
                    {isActive && <motion.div layoutId="wizard-sidebar-indicator" className="absolute left-0 top-2 bottom-2 w-0.5 rounded-full bg-zinc-900" transition={{
                  type: 'spring',
                  stiffness: 400,
                  damping: 30
                }} />}
          ...
          <div className="flex flex-col lg:col-span-9">
            <div className="shrink-0 mb-8 wizard-fade-in">
              {headerEyebrow && t.eyebrowWrap ? <div className={t.eyebrowWrap}>
            ...
            <div ref={stepPanelRef} className={layoutViewportLocked ? `min-h-0 flex-1 overflow-y-auto overscroll-y-contain rounded-2xl border p-4 sm:p-5 ${stepScrollWell}` : `min-h-[380px]`}>
              <AnimatePresence mode="wait" custom={direction}>
                <motion.div key={currentStep} custom={direction} variants={stepVariants} initial="enter" animate="center" exit="exit">
                  {typeof renderStep === 'function' && renderStep(currentStep)}
                </motion.div>
