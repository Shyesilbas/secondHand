        ...
        <div className="bg-slate-50/50 border border-slate-100/80 rounded-2xl p-4 mb-4 shadow-[0_2px_8px_-1px_rgba(15,23,42,0.01)]">
          ...
          <div className="grid grid-cols-2 gap-2.5 py-3 border-y border-slate-100/60 my-4 text-center">
            <div className="border-r border-slate-100/60">
              ...
              <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block mt-1">Rating</span>
            </div>
            <div>
              <span className="text-sm font-bold text-slate-800">{reviewCount}</span>
              <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block mt-1">Reviews</span>
            </div>
          </div>

          {showGreatSeller && (
            <div className="flex items-center gap-2 py-2 px-3 bg-amber-50/50 border border-amber-100/60 rounded-xl mb-4">
              <Award className="w-4 h-4 text-amber-600 shrink-0" />
              <div>
                <span className="text-xs font-bold uppercase tracking-wider text-amber-800">Great Seller Status</span>
                <p className="text-xs text-amber-700/80 font-medium mt-0.5 leading-tight">Highly rated for excellent customer response & delivery speeds.</p>
              </div>
            </div>
          )}
          ...
          {!isOwner ? (
            <ContactSellerButton
              listing={listing}
              className="w-full flex items-center justify-center gap-2 py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-bold transition-all shadow-sm active:scale-95"
            >
              Contact Seller
            </ContactSellerButton>
          ) : (
