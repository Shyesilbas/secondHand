...
@Component
public class VanValidator implements VehicleSpecValidator {

    @Override
    public Result<Void> validate(VehicleListing listing) {
        if (!isType(listing, "VAN")) {
            return Result.success();
        }

        if (listing.getFuelType() == null) {
            return Result.error("Fuel type is required for van listings", "VAN_FUEL_TYPE_REQUIRED");
        }
        if (listing.getMileage() == null || listing.getMileage() < 0) {
            return Result.error("Mileage is required for van listings", "VAN_MILEAGE_REQUIRED");
        }
