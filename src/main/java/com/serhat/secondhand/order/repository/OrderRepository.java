...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @Query("SELECT CASE WHEN COUNT(o) > 0 THEN true ELSE false END FROM Order o WHERE o.user = :user AND o.status = :status")
    boolean existsByUserAndStatus(@Param("user") User user, @Param("status") Order.OrderStatus status);
